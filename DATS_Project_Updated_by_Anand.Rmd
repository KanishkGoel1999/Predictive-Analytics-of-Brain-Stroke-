...
```{r}
brain_stroke_yes$ever_married <- factor(brain_stroke_yes$ever_married, levels = c("1", "2"), labels = c("unmarried", "married"))

contable_marr_str <- table(stroke$ever_married, stroke$stroke)
chitest_marr_str <- chisq.test(contable_marr_str)
chitest_marr_str

brain_stroke_yes_marriedpercentage <- brain_stroke_yes %>%
  group_by(ever_married) %>%
  summarise(Percentage = (n() / total_cases) * 100)

ggplot(brain_stroke_yes_marriedpercentage, aes(x=ever_married, y=Percentage, fill=ever_married)) + 
  geom_bar(stat = "identity", col = "black", width = 0.7) + 
  labs(x = "Ever married or not", y = "Count of brain strokes", title = "Brain Strokes by Ever-married")
